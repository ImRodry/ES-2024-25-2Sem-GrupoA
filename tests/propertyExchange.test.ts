...
describe("Property Exchange Suggestions", () => {
    ...
    it("should suggest beneficial property exchanges", () => {
        ...
        assert.equal(suggestions[0].owner1 !== suggestions[0].owner2, true, "Should suggest exchanges between different owners")
    })
    ...
    it("should consider adjacent properties", () => {
        ...
        assert.equal(
            suggestions.every(s => 
                propertyGraph.get(s.property1.objectId)?.has(s.property2.objectId)
            ),
            true,
            "Should only suggest exchanges between adjacent properties"
        )
